...
// Only difference is rendering of dots
...
class PoincareView extends Ui.View {
	...
	function onLayout(dc) {
		...
		mScr = Ui.loadResource(Rez.JsonData.jsonPoinD);
    ...
    function onUpdate(dc) {
		...
		for (var i=0; i < 3; i++) {
			dc.drawRectangle(mScr[20], mScr[16+i], mScr[19], 2);
		}
		for (var i=0; i < 3; i++) {
			dc.drawRectangle(mScr[21+i], mScr[25], 2, mScr[24]);
		}
		
		dc.setColor( $.mLabelColour, Gfx.COLOR_TRANSPARENT);
		dc.drawText( mScr[0], mScr[1], mTitleFont, mTitleLabels[viewToShow-1], mJust);
		// draw "RR ms"
		dc.drawText( mScr[2], mScr[3], mLabelFont, mLabelInterval, mJust);
		...
		dc.drawText( mScr[10], mScr[11], mLabelFont, format(" $1$ ",[ceil.format("%d")]), mJust);
		dc.drawText( mScr[12], mScr[13], mLabelFont, format(" $1$ ",[mid.format("%d")]), mJust);	
		dc.drawText( mScr[14], mScr[15], mLabelFont, format(" $1$ ",[floor.format("%d")]), mJust);
			
		// Draw the data
		
		// set colour of rectangles. can't see white on white :-)
		if ($.bgColSet == 3) { // BLACK
			dc.setColor(Gfx.COLOR_WHITE, $.mBgColour);
		} else {
			dc.setColor(Gfx.COLOR_BLACK, $.mBgColour);
		}
...
class PoincareView extends Ui.View {
	...
	function fBuildJson() {
		...
		// build JSON string
		var mStr = "<jsonData id="+"jsonPoinDisp"+mScaleY+">[";
...
// old code for asymmetric scales
		...
		// iterate through available data drawing rectangles as less expensive than circles
		// reduce number of array accesses
		//var previousSample = $.mIntervalSampleBuffer[1];
		// can't do same with x value as maybe different scale factors
		...
		//for( var i=2; i < mNumberEntries; i++ ){
			// Plot y = RR(i+1), x = RR(i) (or i and i-1)
			// should use getSample() in case of circular buffer implemented
			//var sampleN = $.mIntervalSampleBuffer[i]; // x axis value to plot
			//var sampleN1 = $.mIntervalSampleBuffer[i]; // y axis value to plot
			// work out x and y from numbers and scales
